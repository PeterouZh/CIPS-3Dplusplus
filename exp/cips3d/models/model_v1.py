...
class StyledConv(nn.Module):
  def __init__(self,
    ...
    self.activate = FusedLeakyReLU(out_channel, scale=1.)
    pass
...
class ConvLayer(nn.Sequential):
  def __init__(self, in_channel, out_channel, kernel_size, downsample=False,
    ...
    if activate:
      layers.append(FusedLeakyReLU(out_channel, bias=bias, scale=1.))
...
class Decoder(nn.Module):
  ...
  def forward(self,
              ...
              noise=None,
              mesh_path=None):
    # with torch.set_grad_enabled(not return_latents):
    #   latent, noise = self.styles_and_noise_forward(styles,
    #                                                 noise,
    #                                                 inject_index,
    #                                                 truncation,
    #                                                 truncation_latent,
...
class Generator(nn.Module):
  ...
  def forward(self,
    ...
    if N_rays_grad is not None and N_rays_grad < H * W:
      ...
      thumb_rgb, sdf, mask, xyz, rgb, eikonal_term = self.rays_forward(
        N_rays_forward=N_rays_forward,
        N_samples_forward=N_samples_forward,
        pts=pts,
        # normalized_pts=normalized_pts,
  ...
  def rays_forward(self,
    ...
    for idx in range(0, N_rays, N_rays_forward):
      ...
      if self.enable_decoder:
        if self.renderer_detach:
          features = features.detach()
        if self.predict_rgb_residual:
          features = features * (1 - mask.detach())
        features = rearrange(features, "b hw c -> b c hw 1").contiguous() # bug

        rgbd_in = None
        if self.predict_rgb_residual:
          thumb_rgb_bg = thumb_rgb.detach() * mask.detach()
          rgbd_in = rearrange(thumb_rgb_bg, "b hw c -> b c hw 1").contiguous()
    ...
    return thumb_rgb, sdf, mask, xyz, rgb, eikonal_term
  ...
  def create_mapping_decoder(self,
    ...
    for i in range(N_layers - 1):
      if i != N_layers - 2:
        layers.append(EqualLinear(style_dim, style_dim, lr_mul=lr_mul_mapping, activation="fused_lrelu"))
        layers.append(nn.LayerNorm(style_dim))
      else:
        layers.append(EqualLinear(style_dim, style_dim, lr_mul=lr_mul_mapping, activation=None))
        layers.append(nn.LayerNorm(style_dim))
