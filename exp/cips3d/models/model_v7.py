...
from exp.cips3d.models.volume_renderer_v7 import VolumeFeatureRenderer
...
class Decoder(nn.Module):
  ...
  def create_synthesis(self):
    ...
    for i in range(self.log_in_size + 1, self.log_size + 1): # [3, 11)
      ...
      _in_dim = _out_dim
      _out_dim = self.channels[_cur_size]

      if _cur_size in self.upsample_list:
        upsample = True
      else:
        upsample = False
      ...
      self.convs.append(
        StyledConv(
          in_channel=_in_dim,
          out_channel=_out_dim,
          kernel_size=self.kernel_size if _cur_size not in self.filter_list else self.filter_size,
          style_dim=self.style_dim,
          upsample=upsample,
          blur_kernel=self.blur_kernel,
          project_noise=self.project_noise,
      ...
      self.convs.append(
        StyledConv(in_channel=_out_dim,
                   out_channel=_out_dim,
                   kernel_size=self.kernel_size if _cur_size not in self.filter_list else self.filter_size,
                   style_dim=self.style_dim,
                   blur_kernel=self.blur_kernel,
  ...
  def forward(self,
    ...
    if enable_decoder:
      ...
      rgb = self.decoder(features=features,
                         ...
                         rgbd_in=rgbd_in if self.predict_rgb_residual else None,
