...
from exp.cips3d.models.volume_renderer_v8 import VolumeFeatureRenderer, TriplaneRenderer
...
class BackboneMapping(nn.Module):
  def __init__(self,
    ...
    self.z_norm = PixelNorm()
    self.cond_norm = nn.Sequential(
      EqualLinear(cond_dim, style_dim, lr_mul=lr_mul_mapping, activation="fused_lrelu"),
      PixelNorm(),
    )

    layers = [
      EqualLinear(z_dim + style_dim, style_dim, lr_mul=lr_mul_mapping, activation="fused_lrelu"),
    ]
...
class Generator_utils(object):
  ...
  def get_mean_latent(self,
    ...
    cond = self.cond.expand(N_noises, -1)
    style_backbones = self.style_backbone(z=noises_backbone,
                                          cond=cond)
  ...
  def mapping_backbone(self,
    ...
    styles = self.style_backbone(z=latents,
                                 cond=cond)
  ...
  def mapping_networks(self,
    ...
    if truncation < 1:
      if recompute_mean or \
        ...
        style_backbone_mean, style_decoder_mean = self.get_mean_latent(N_noises=10000,
                                                                       device=zs[0].device)
...
class Generator(nn.Module, Generator_utils):
  ...
  def __init__(self,
    ...
    cam_extrinsics, _, _, _, _ = nerf_utils.Camera.generate_camera_params(
      locations=torch.zeros(1, 2, device='cpu'),
      device='cpu',
      batch=1,
      **{**G_kwargs.cam_cfg})
    self.register_buffer('cond', cam_extrinsics.flatten(start_dim=1))
    ...
    self.decoder = Decoder(style_dim=mapping_decoder_cfg['style_dim'],
                           **{**decoder_cfg,
                              'in_channel': self.renderer.out_dim})
  ...
  def forward(self,
    ...
    if cond is None or (isinstance(cond, str) and cond.lower() == 'none'):
      cond = cam_poses.flatten(start_dim=1)
    elif cond == 'const':
      cond = self.cond.expand(cam_poses.shape[0], -1)
    ...
    planes = self.backbone(styles=style_backbone, noise=0).contiguous()  # bug
  ...
  def create_mapping_backbone(self,
    ...
    self.style_backbone = BackboneMapping(z_dim=z_dim,
                                          style_dim=style_dim,
                                          lr_mul_mapping=lr_mul_mapping,
                                          N_layers=N_layers,
                                          cond_dim=12)
