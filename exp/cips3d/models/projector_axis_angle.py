...
class StyleGAN2Projector(object):
  ...
  def get_perceptual_fea(self,
    ...
    if image_tensor_thumb is None:
      image_tensor_thumb = F.interpolate(image_tensor, scale_factor=64/1024,
  ...
  def _G_forward(self,
    ...
    if rot is not None and trans is not None:
      trans = F.normalize(trans, p=2, dim=1) # on sphere
      cam_extrinsics = nerf_utils.Camera.get_camera2world(cam2world=rot, trans=trans)

      trajectory = torch.zeros(cam_extrinsics.shape[0], 2, device=device)
      _, sample_focals, sample_near, sample_far, _ = nerf_utils.Camera.generate_camera_params(
        locations=trajectory,
        device=device,
        **{**cam_cfg})
    else:
    ...
    if return_mask:
      ...
      return ret_maps['rgb'], ret_maps['thumb_rgb'], mask, mask_thumb
    else:
  ...
  def _create_cam_optimizer(self,
    ...
    return rot, trans, optimizer_cam
...
class StyleGAN2Projector_Flip(StyleGAN2Projector):
  ...
  def project_wplus(
    ...
    rot, trans, optimizer_cam = self._create_cam_optimizer(optim_cam=optim_cam, lr_cam=lr_cam, bs=bs_cam)
    ...
    for step in pbar:
      ...
      if step < N_steps_pose:  # optimizing cam, render
        torch_utils.set_optimizer_lr(optimizer=optimizer_decoder, lr=0)
        w_noise_scale = (1 - min(step / (N_steps_pose + 1e-5), 1)) * 0.0
      ...
      synth_images, synth_images_thumb, masks, masks_thumb = self._G_forward(
        G=G,
        ...
        style_render_noise=w_noise_render,
        style_decoder_noise=w_noise_decoder,
        rot=rot + w_noise_scale * 0.3 * torch.randn_like(rot),
        trans=trans + w_noise_scale * 0.15 * torch.randn_like(trans),
        return_mask=True,
      ...
      if st_web and (step % st_log_every == 0 or step == N_steps - 1):
        ...
        merged_pil = pil_utils.merge_image_pil([target_pil, img_pil, mask_pil,
                                                target_pil_flip, img_pil_flip, mask_pil_flip], nrow=3)
        pil_utils.add_text(
          merged_pil, text=f"step: {step}, psnr: {psnr:.2f}dB, ssim: {ssim:.2f}", size=merged_pil.size[0] // 20)

        st_utils.st_image(merged_pil, caption=f"target {target_pil.size}, img_pil {img_pil.size}",
                          debug=global_cfg.tl_debug, st_empty=st_image)
        st_text_rot.write(f"{rot}")
        st_text_trans.write(f"{trans}")
        video_f_target_proj.write(merged_pil)
