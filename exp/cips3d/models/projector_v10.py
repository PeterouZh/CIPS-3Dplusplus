...
def pad_image(img):
  ...
  if W > H:
    ...
    img = trans_f.pad(img, padding=padding, padding_mode=padding_mode)
  else:
    padding = (H - W) // 2
...
def load_pil_crop_resize(image_path,
  ...
  # target_pil, padding = pad_image(target_pil)
  target_pil = trans_f.center_crop(target_pil, min(target_pil.size))
  padding = (0, 0, 0, 0)

  W, H = target_pil.size

  padding = (int(padding[0]/W * out_size),
             int(padding[1]/H * out_size),
             int(padding[2]/W * out_size),
             int(padding[3]/H * out_size))

  if target_pil.size == (out_size, out_size):
    return target_pil, padding
...
class StyleGAN2Projector(object):
  ...
  def _create_cam_optimizer(self,
    ...
    azim.data[:, 0].copy_(torch.tensor(azim_init[:bs]))
...
class StyleGAN2Projector_Flip(StyleGAN2Projector):
  ...
  def _get_target_image(self,
                        ...
                        out_size=1024):
    # Load target image.
    target_pil, padding = load_pil_crop_resize(image_path, out_size=out_size)
    # [0, 1]
    target_np = np.array(target_pil, dtype=np.float32).transpose([2, 0, 1]) / 255.
    # [-1, 1]
    target_images = (torch.from_numpy(target_np).to(device) - 0.5) * 2
    ...
    return target_pil, target_np, target_images, target_pil_flip, padding
  ...
  def _create_cam_optimizer(self,
    ...
    # azim = torch.zeros(bs, 1).cuda()
    azim = -np.pi + torch.rand(bs, 1).cuda() * np.pi * 2
  ...
  def project_wplus(
    ...
    target_pil, target_np, target_images, target_pil_flip, padding = self._get_target_image(
      image_path=image_path, device=device, out_size=img_size)
    ...
    if os.path.isfile("datasets/cars/style_render.pkl"):
      loaded_style_render = torch_utils.torch_load("datasets/cars/style_render.pkl", rank=0)
      w_render_opt.data.copy_(torch.lerp(style_render_mean, loaded_style_render, 0.9))
    ...
    for step in pbar:
      ...
      if step < N_steps_pose:  # optimizing cam, render
        torch_utils.set_optimizer_lr(optimizer=optimizer_decoder, lr=0)
        # w_noise_scale = (1 - min(step / (N_steps_pose + 1e-5), 1))
        w_noise_scale = 0
      ...
      synth_images, synth_images_thumb, masks, masks_thumb, cam_extrinsics = self._G_forward(
        ...
        style_render_noise=w_noise_render,
        style_decoder_noise=w_noise_decoder,
        rot=azim + w_noise_scale * np.pi / 4 * torch.randn((bs_cam, 1), device=device),
        trans=elev + w_noise_scale * 0.1 * torch.rand(1, device=device),
        return_mask=True,
      ...
      if st_web and (step % st_log_every == 0 or step == N_steps - 1):
        ...
        cam_pose_pil = cam_pose_pil.resize((img_size, img_size))
