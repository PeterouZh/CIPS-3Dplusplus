...
from tl2.proj.pytorch.downsampler import create_pil_lanczos_layer
...
class StyleGAN2Projector(object):
  ...
  def get_perceptual_fea(self,
    ...
    if image_tensor_thumb is None:
      ...
      if not hasattr(self, 'down_layer'):
        down_layer = create_pil_lanczos_layer(downsample_factor=img_size // 64, device=image_tensor.device)
        down_layer.requires_grad_(False)
        self.down_layer = down_layer
      with torch.no_grad():
        image_tensor_thumb = self.down_layer(image_tensor)
  ...
  def _G_forward(self,
    ...
    if return_mask:
      ...
      mask = F.interpolate(mask_thumb, scale_factor=(ret_maps['rgb'].shape[-1] / mask_thumb.shape[-1]),
                           recompute_scale_factor=False, mode='bicubic', align_corners=False)
...
class StyleGAN2Projector_Flip(StyleGAN2Projector):
  ...
  def project_wplus(
    ...
    if st_web:
      ...
      st_chart_psnr = st_utils.LineChart(x_label='step', y_label='psnr')
      st_chart_ssim = st_utils.LineChart(x_label='step', y_label='ssim')
      st_chart_gradnorm_cam = st_utils.LineChart(x_label='step', y_label='GradNorm_cam')
      st_chart_gradnorm_render = st_utils.LineChart(x_label='step', y_label='GradNorm_render')
      st_chart_gradnorm_decoder = st_utils.LineChart(x_label='step', y_label='GradNorm_decoder')
    ...
    for step in pbar:
      ...
      Grad_norm_cam = torch.nn.utils.clip_grad_norm_(
        optimizer_cam.param_groups[0]['params'], global_cfg.grad_clip_cam)
      Grad_norm_render = torch.nn.utils.clip_grad_norm_(
        optimizer_render.param_groups[0]['params'], global_cfg.grad_clip_render)
      Grad_norm_decoder = torch.nn.utils.clip_grad_norm_(
        optimizer_decoder.param_groups[0]['params'], global_cfg.grad_clip_decoder)
      ...
      if st_web and step >= 50 and (step % st_log_every == 0 or step == N_steps - 1):
        ...
        st_chart_reg_loss.write(x=step, y=reg_loss.item())
        st_chart_loss.write(x=step, y=loss.item())
        st_chart_gradnorm_cam.write(x=step, y=Grad_norm_cam.item())
        st_chart_gradnorm_render.write(x=step, y=Grad_norm_render.item())
        st_chart_gradnorm_decoder.write(x=step, y=Grad_norm_decoder.item())
