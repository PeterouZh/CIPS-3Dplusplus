...
from .projector_v10 import StyleGAN2Projector, StyleGAN2Projector_Flip
...
class STModel(object):
  ...
  def _flip_inversion_web(self,
    ...
    network_pkl = st_utils.selectbox_v1('network_pkl', cfg.network_pkl,
    ...
    projector.project_wplus(
      outdir=outdir,
      image_path=image_path,
      img_size=cam_cfg.img_size * (2 ** len(g_ema.decoder.upsample_list)),
      cam_cfg=cam_cfg,
      nerf_cfg=nerf_cfg,
      st_log_every=st_log_every,
      st_web=True,
  ...
  def _sample_multi_view_web(self,
    ...
    light_location_back = copy.deepcopy(light_location)
    light_location_back[-1] = - light_location[-1]

    st_utils.st_set_sep('Sampling kwargs')
    seed = st_utils.get_seed(cfg.seeds)
    view_mode = st_utils.selectbox('view_mode', cfg.view_mode, default_value=cfg.default_view_mode, sidebar=True)
    only_rotate = st_utils.checkbox('only_rotate', cfg.only_rotate, sidebar=True)
    if view_mode == 'yaw':
      azim_range = st_utils.parse_list_from_st_text_input('azim_range', cfg.azim_range, sidebar=True)
      elev = st_utils.number_input('elev', cfg.elev, sidebar=True, format="%.3f")
    ...
    for j in tqdm(range(0, N_frames, chunk)):
      with torch.no_grad():
        ...
        # if trajectory[j, 0].item() > np.pi / 2 or trajectory[j, 0].item() < - np.pi / 2:
        #   cur_light_location = light_location_back
        # else:
        #   cur_light_location = light_location

        cur_light_location = [
          5 * np.sin(trajectory[j, 0].item()),
          0,
          5 * np.cos(trajectory[j, 0].item()),
        ]

        renderer = create_mesh_renderer(cameras=cameras,
                                        image_size=512,
                                        light_location=(cur_light_location, ),
                                        specular_color=((0.2, 0.2, 0.2),),
                                        ambient_color=((0.1, 0.1, 0.1),),
                                        diffuse_color=((0.65, .65, .65),),
                                        # light_location=(light_location, ),
...
class STModelCars(STModel):
  ...
  def _flip_inversion_web(self,
    ...
    cfg.st_log_every = st_utils.number_input('st_log_every', cfg.st_log_every, sidebar=True)
    ...
    # cfg.img_size = st_utils.number_input('img_size', cfg.img_size, sidebar=True)
    cfg.azim_init = st_utils.parse_list_from_st_text_input('azim_init', cfg.azim_init, sidebar=True)
  ...
  def _render_multi_view_web(self,
    ...
    if view_mode == 'yaw':
      azim_range = st_utils.parse_list_from_st_text_input('azim_range', cfg.azim_range, sidebar=True)
      elev_range = st_utils.parse_list_from_st_text_input('elev_range', cfg.elev_range, sidebar=True)
      fix_elev = st_utils.checkbox('fix_elev', cfg.fix_elev, sidebar=True)
    ...
    saved_w_file = st_utils.text_input('saved_w_file', cfg.saved_w_file, sidebar=True)
    # w_gallery = st_utils.selectbox_v1('w_gallery', cfg.w_gallery, default_key=cfg.default_w_gallery, sidebar=True)
    w_gallery = st_utils.radio('w_gallery', cfg.w_gallery.keys(), sidebar=True, value_dict=cfg.w_gallery,
                               default_value=cfg.default_w_gallery)
    use_w_gallery = st_utils.checkbox('use_w_gallery', cfg.use_w_gallery, sidebar=True)
    if use_w_gallery:
      ws = os.listdir(w_gallery)
      ws = st_utils.selectbox('ws', ws, sidebar=True)
      # saved_w_file = f"{w_gallery}/{ws}/{ws}_w.pth"
      saved_w_file = f"{w_gallery}/{ws}/w.pth"
    ...
    if view_mode == 'yaw':
      ...
      azim = azim_range[0] + (azim_range[1] - azim_range[0]) * np.sin(t * np.pi)
      # azim = azim_range[0] + (azim_range[1] - azim_range[0]) * t
      elev = elev_range[0] + (elev_range[1] - elev_range[0]) * np.sin(t * np.pi)
    ...
    if fix_elev:
      trajectory[:, 1].fill_(loaded_elev)
    ...
    for j in tqdm(range(0, N_frames, chunk)):
      with torch.no_grad():
        ...
        if j == 0:
          cam_pose_vis = CameraPoseVisualizer(figsize=(1024 / 100, 1024 / 100))
        ...
        cam_pose_vis.extrinsic2pyramid(extrinsic=sample_cam_extrinsics[j].cpu().numpy(), color=cam_pose_vis.cmap(1.))
        cam_pose_pil = cam_pose_vis.to_pil()
        cam_pose_pil = pil_utils.pil_resize(cam_pose_pil, rgb_source_pil.size)
