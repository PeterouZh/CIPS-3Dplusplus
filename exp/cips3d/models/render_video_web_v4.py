...
class STModel(object):
  ...
  def _sample_multi_view_web(self,
    ...
    mesh_size = st_utils.number_input('mesh_size', G_kwargs.cam_cfg.img_size, sidebar=True)
    fixed_mesh = st_utils.checkbox('fixed_mesh', False, sidebar=True)
    cond = st_utils.selectbox('cond', options=['const', 'none'], default_value='const', sidebar=True)
    ...
    if view_mode == 'circle':
      ...
      elev = global_cfg.elev_mean + global_cfg.elev_std * np.cos(t * 2 * np.pi)
    ...
    for j in tqdm(range(0, N_frames, chunk)):
      with torch.no_grad():
        ...
        ########## Extract surface ##########
        if not fixed_mesh or mesh is None:
          with torch.no_grad():
            mesh_ret_maps = g_ema(zs=sample_z,
                                  cond=cond,
                                  style_render=None,
                                  style_decoder=None,
                                  cam_poses=sample_cam_extrinsics[j:j + chunk],
