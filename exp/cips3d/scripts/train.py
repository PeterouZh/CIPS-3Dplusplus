...
def update_G(opt,
  ...
  if enable_decoder and g_regularize:
    generator.zero_grad(set_to_none=True)
    ...
    for j in range(0, path_batch_size, opt.chunk):
      ...
      ret_maps = generator(zs=curr_path_noise,
...
def sphere_init_func(opt,
  ...
  pbar = tl2_utils.TL_tqdm(total=10000, start=0, desc='sphere_init_func')
  ...
  for idx in pbar:
    ...
    sdf, target_values = g_module.init_forward(zs=noise,
    ...
    if (idx + 1) % 100 == 0 or global_cfg.tl_debug:
