...
from tl2.proj.pytorch import downsampler

from stylesdf.models.distributed import get_rank, synchronize, \
  reduce_loss_dict, reduce_sum, get_world_size
...
def update_D_render(discriminator,
  ...
  if real_imgs.shape[-1] > gen_imgs.shape[-1]:
    ...
    if not hasattr(downsampler, 'down_layer'):
      down_layer = downsampler.create_pil_lanczos_layer(
        downsample_factor=real_imgs.shape[-1] // gen_imgs.shape[-1],
        device=real_imgs.device)
      down_layer.requires_grad_(False)
      setattr(downsampler, 'down_layer', down_layer)
    with torch.no_grad():
      real_imgs = downsampler.down_layer(real_imgs)
...
def save_models(g_module,
  ...
  torch_utils.save_models(save_dir=saved_dir, model_dict=model_dict, save_module=False)
...
def main():
  ...
  # setup args
  if global_cfg.tl_debug:
    opt.training.batch = 2
    opt.training.chunk = 2

  mem_gpu = torch_utils.get_gpu_memory_GB()
  if mem_gpu < 16: # debug
    # opt.training.batch = 2
    # opt.training.chunk = 2
    # global_cfg.init_iters = 100
    # global_cfg.G_kwargs.nerf_cfg.N_samples = 24 # debug
    pass
