...
from tl2.proj.tools3d import compute_normal_from_depth
...
def eikonal_loss(eikonal_term,
  ...
  if eikonal_term == None:
    ...
    # eikonal_loss = ((eikonal_term - 1) ** 2).mean()
  ...
  if global_cfg.get('sdf_even_loss', False):
    eikonal_loss = ((sdf[..., 0:-1, :] - sdf[..., 1:, :]) - global_cfg.sdf_even_interval).abs().mean()

  if global_cfg.get('depth_reg', False):
    img_size = depth.shape[-1]
    intrinsic = torch.tensor([[img_size/2, 0,          img_size/2, 0],
                              [0,          img_size/2, img_size/2, 0],
                              [0,          0,          1,          0],
                              [0,          0,          0,          1]], device=depth.device)
    intrinsic = intrinsic.unsqueeze(0).expand(depth.shape[0], -1, -1)
    generated_norm = compute_normal_from_depth.get_normal_from_depth(depth_torch=depth.permute(0, 2, 3, 1),
                                                                     intrinsic=intrinsic)
    eikonal_loss = ((generated_norm.norm(p=2, dim=3) - 1) ** 2).mean()
...
def update_G(opt,
  ...
  G_grad_norm_backbone = torch.nn.utils.clip_grad_norm_(
    chain(g_module.backbone.parameters(), g_module.style_backbone.parameters()),
    global_cfg.grad_clip)
  loss_dict['grad_norm']['G_grad_norm_backbone'] = G_grad_norm_backbone.item()
  
  G_grad_norm_renderer = torch.nn.utils.clip_grad_norm_(
    g_module.renderer.parameters(),
    global_cfg.grad_clip)
  loss_dict['grad_norm']['G_grad_norm_renderer'] = G_grad_norm_renderer.item()
  ...
  if (enable_decoder and g_regularize) or global_cfg.tl_debug:
    ...
    G_grad_norm_backbone_path_reg = torch.nn.utils.clip_grad_norm_(
      chain(g_module.backbone.parameters(), g_module.style_backbone.parameters()), 0)
    G_grad_norm_renderer_path_reg = torch.nn.utils.clip_grad_norm_(
      g_module.renderer.parameters(), 0)
    G_grad_norm_decoder_path_reg = torch.nn.utils.clip_grad_norm_(
      chain(g_module.decoder.parameters(), g_module.style_decoder.parameters()),
      global_cfg.grad_clip)
    loss_dict['grad_norm']['G_grad_norm_backbone_path_reg'] = G_grad_norm_backbone_path_reg.item()
...
def sphere_init_func(opt,
  ...
  N_total = global_cfg.get('init_iters', 10000)
  ...
  for idx in pbar:
    noise = mixing_noise(bs, generator.z_dim, opt.mixing, device)
    cam_extrinsics, focal, near, far, gt_viewpoints = nerf_utils.Camera.generate_camera_params(
      device=device,
      batch=bs,
      **global_cfg.G_kwargs.cam_cfg)
...
def get_sample_kwargs_views(g_module,
                            device):
...
def save_images_random(g_ema,
  ...
  truncation = 0.7
  batch = sample_cam_extrinsics.shape[0]
  N_row = int(math.sqrt(batch))
  ...
  for k in range(0, batch, step_size):
    curr_noise = [n[k:k + step_size] for n in sample_z]
    ret_maps = g_ema(zs=curr_noise,
                     cond='const',
                     cam_poses=sample_cam_extrinsics[k:k + step_size],
  ...
  Gz_max2keep = tl2_utils.MaxToKeep.get_named_max_to_keep(name='random_samples_G_z', max_to_keep=4,
                                                          use_circle_number=False)
  Gz_nerf_max2keep = tl2_utils.MaxToKeep.get_named_max_to_keep(name='random_samples_G_z_nerf', max_to_keep=4,
                                                               use_circle_number=False)

  os.makedirs(saved_dir, exist_ok=True)

  saved_path = f"{saved_dir}/{step:09d}.jpg"
  Gz_max2keep.step(saved_path)
  utils.save_image(samples,
                   saved_path,
                   padding=0,
                   nrow=N_row,
  ...
  saved_path = f"{saved_dir}/{step:09d}_nerf.jpg"
  Gz_nerf_max2keep.step(saved_path)
  utils.save_image(thumbs_samples,
                   saved_path,
                   padding=0,
                   nrow=N_row,
...
def get_sample_kwargs_random(g_module,
                             device,
                             N_row,
                             N_col):
  batch = N_row * N_col
  ...
  G_kwargs.cam_cfg['azim_range'] = 0.
  G_kwargs.cam_cfg['elev_range'] = 0.
  # G_kwargs.cam_cfg.img_size = img_size

  locations = torch.zeros(batch, 2, device=device)
  locations[:, 0] = 0 * (math.pi / 2)

  sample_z = mixing_noise(batch, g_module.z_dim, 0.9, device)
...
def train(opt,
  ...
  sample_kwargs = get_sample_kwargs_views(g_module=g_module, device=device)
  sample_kwargs_random = get_sample_kwargs_random(g_module=g_module, device=device,
                                                  N_row=8, N_col=8)
  state_dict['sample_kwargs_random'] = sample_kwargs_random

  # for idx in tqdm(range(10)):
  #   save_images_random(g_ema=g_ema,
  #                      saved_dir=global_cfg.tl_imgdir,
  #                      sample_kwargs=sample_kwargs_random,
  #                      step=idx)
  # exit(0)
  ...
  for idx in pbar:
    ...
    if not global_cfg.skip_eval and \
            ((idx + 1) % global_cfg.log_ckpt_every == 0 or idx == 0 or global_cfg.tl_debug):
      # sync weights
      ddp_utils.sync_models(rank=rank, world_size=world_size, sync_models=sync_models)
      ...
      setup_evaluation(rank=rank,
                       world_size=world_size,
                       batch_gpu=global_cfg.get('batch_gpu_eval', 64),
                       distributed=world_size > 1,
                       real_dir=real_dir,
...
def create_optims(opt,
  ...
  for name, value in params_dict_g.items():
    if name.startswith(('decoder', 'style_decoder.', 'backbone', 'style_backbone.')):
...
def create_dataset(opt,
  ...
  dataset = MultiResolutionDataset(global_cfg.obs_training_dataset.datapath,
                                   ...
                                   hflip=global_cfg.get('hflip', True))
...
def main():
  ...
  if global_cfg.tl_resume:
    ...
    if 'sample_kwargs_random' in state_dict_loaded:
      del state_dict_loaded['sample_kwargs_random']
