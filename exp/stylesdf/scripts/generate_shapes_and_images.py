  ...
  for i in tqdm(range(opt.identities)):
    with torch.no_grad():
      ...
      utils.save_image(rgb_images,
                       os.path.join(global_cfg.tl_outdir, f'{i:07d}.png'),
                       nrow=num_viewdirs,
                       normalize=True,
                       padding=0,
                       value_range=(-1, 1), )
      ...
      utils.save_image(rgb_images_thumbs,
                       os.path.join(global_cfg.tl_outdir, f'{i:07d}_thumb.png'),
                       nrow=num_viewdirs,
                       normalize=True,
                       padding=0,
                       value_range=(-1, 1), )
      ...
      if not opt.no_surface_renderings:
        ...
        for j in range(0, num_viewdirs, surface_chunk):
          ...
          for k in range(surface_chunk):
            ...
            depth_mesh_filename = os.path.join(global_cfg.tl_outdir, f'sample_{i}_depth_mesh{loc_str}.obj')
            ...
            if j == 0:
              ...
              if marching_cubes_mesh != None:
                marching_cubes_mesh_filename = os.path.join(global_cfg.tl_outdir, f'sample_{i}_marching_cubes_mesh{loc_str}.obj')
