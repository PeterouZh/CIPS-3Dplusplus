...
class VolumeFeatureRenderer(nn.Module):
  ...
  def get_rays(self,
    ...
    dirs = torch.stack(
      tensors=[(self.i - self.out_im_res * .5) / focal,
               -(self.j - self.out_im_res * .5) / focal,
               -torch.ones_like(self.i).expand(focal.shape[0], self.out_im_res, self.out_im_res)],
      dim=-1)
  ...
  def forward(self,
    ...
    rgb, features, sdf, mask, xyz, eikonal_term = self.render(
      focal,
      c2w=cam_poses,
      near=near,
      far=far,
